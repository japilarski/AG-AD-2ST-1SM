...
knitr::kable(completed_data %>% head(10))
```
...
#Stworzonie kategorii dla cen poprzez średnią i odchylenie standardowe
...
```
Poniższy wykres kolumnowy przedstawia rozklad cen mieszkań z podziałem co 1000. Można wywnioskować, iż tak jak na wykresie kołowym, większość mieszkań jest blisko średniej ceny rynkowej. Tańsze mieszkania oraz droższe są w zdecydowanej mniejszości. Dodatkowo zaznaczono liniami jak ksztaltują się ceny mieszkań w podziale na ilość sypialni.
...
completed_data$area_category <- cut(
  completed_data$area, 
  breaks = c(0, 2000, 5000, 10000, 16200),  # Granice przedziałów
  labels = c("0-2000", "2001-5000", "5001-10000", "10001-1620"),  # Nazwy przedziałów
  right = TRUE  # Prawostronne przedziały (np. 0-50 oznacza <=50)
)
